...
function doRectanglesOverlap(rect1, rect2) {
  const isOverlapX = rect2.left - rect1.left < rect1.width;
  const isOverlapY = rect2.top - rect1.top < rect1.height;
...
function isCreditCardNumber(ccn) {
  const validateDigit = (d, i) => {
    const digit = i % 2 !== 0 ? +d * 2 : +d;
    return digit > 9 ? digit - 9 : digit;
  };
  const sum = `${ccn}`.split('').reverse().reduce((acc, d, i) => acc + validateDigit(d, i), 0);
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  const sum = `${num}`.split('').reduce((acc, n) => acc + +n, 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.includes(str[i])) {
      ...
      return false;
    } else if (pairs[str[i]] === stack[stack.length - 1]) {
      stack.pop();
    } else {
      return false;
    }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < h; i += 1) {
    mr.push([]);
    for (let j = 0; j < w; j += 1) {
      const sum = m1[i].reduce((acc, item, k) => acc + item * m2[k][j], 0);
      mr[i].push(sum);
    }
  }
